        ...
        if ($parent) {
            if ($menu->childs->count() > 0) {
                foreach ($menu->childs as $menu_child) {
                    if (\Schema::hasTable($menu_child->table_map) && \Schema::hasColumn($menu_child->table_map, 'is_read')) {
                        $count += DB::table($menu_child->table_map)->whereNull('is_read')->orWhere('is_read', 0)->count();
                    }
                }
            }
        } else {
            if (\Schema::hasTable($menu->table_map) && \Schema::hasColumn($menu->table_map, 'is_read')) {
