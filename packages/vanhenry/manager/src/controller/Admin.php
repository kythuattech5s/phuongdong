...
class Admin extends BaseAdminController
{
    ...
    public function crawlExcute()
    {
        ...
        if (empty($get['link']) || strlen(trim($get['link'])) == 0) {
            return redirect()->back()->with('typeNotify', 'error')->with('messageNotify', 'Bạn phải nhập link sản phẩm trên digikey');
        }
        $linkCrawl = $get['link'];
        $responses = $this->curl($linkCrawl);
        if (!isset($responses['response'])) {
            return redirect()->back()->with('typeNotify', 'error')->with('messageNotify', 'Không thể lấy dữ liệu');
        }
        ...
        if (!$nameProduct) {
            return redirect()->back()->with('typeNotify', 'error')->with('messageNotify', 'Không thể lấy tên sản phẩm');
        }
        ...
        if (count($prosIsExists) > 0) {
            return redirect()->back()->with('typeNotify', 'error')->with('messageNotify', 'Sản phẩm với mã sản phẩm "'.$data['code'].'" đã tồn tại');
        }
