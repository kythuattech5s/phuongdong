...
var COMMENT = (function(){
	...
	function showFormRepComment(formGroup,element){
		...
		textArea.addEventListener('keydown',function(e){
			...
			if(arrayKey.find((value)=>{ return value == e.which})){
				key.push(e.which);
			};
			if(key[0] == 13){
				e.preventDefault();
	...
	function loadMore(){
		...
		if(btnMore){
			btnMore.onclick = function(){
				...
				datas._token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
				if(_hasFilter){
					var input = document.querySelector('.comment-filter__lists input:checked');
					if(input){
	...
	// CALLBACK
    function receivedComment(response, dataForm, hasNotification = true, insertType = 'item'){
		if(hasNotification){
			notification(response);
		}
		...
		if(response.lastPage){
			processingButtonLoadMore(response,{table:dataForm.map_table,id:dataForm.map_id});
		}
		if(response.html){
			if(insertType == 'item'){
	...
	function processingButtonLoadMore(res,data){
		...
		if(res.lastPage == 1){
			...
			if(!buttonLoadMore){
				...
				document.querySelector('.comment-box__list').insertAdjacentElement('afterend',buttonLoadMore);
			}
		}
	}
		...
		inputs.forEach(function(input,index){
			input.onchange = function(e){
				XHR.send({
					...
					processingButtonLoadMore(res,{table:filterLists.dataset.table,id:filterLists.dataset.id})
					return res;
				}).then((res)=>{
					receivedComment(res,{map_table:filterLists.dataset.table,map_table:filterLists.dataset.id},false,'list');
				})
			}
		})
	}
